...
public class AppointmentService {
    ...
    public Page<AppointmentResponse> listAppointments(int page,
        ...
        if (appointmentDate != null || doctorId != null || status != null || type != null) {
            return repository.findByOptionalFilters(appointmentDate, doctorId, status, type, pageable)
    ...
    public AppointmentResponse getAppointmentById(String appointmentId) {
        Appointment appt = repository.findById(appointmentId)
                .orElseThrow(() -> new BadRequestAlertException("Appointment not found: " + appointmentId,
    ...
    public java.util.List<AvailableSlotResponse> getAvailableSlots(String doctorId, java.time.LocalDate date) {
        ...
        java.util.List<DentistWorkSchedule> schedules = dentistWorkScheduleRepository
                .findByDentistIdAndWorkDate(doctorId, date);
        schedules.removeIf(s -> s.getStatus() == DentistWorkScheduleStatus.CANCELLED
                || s.getStatus() == DentistWorkScheduleStatus.EXPIRED);
    ...
    public AppointmentResponse schedule(CreateAppointmentRequest request) {
        ...
        if (request.getAppointmentDate().isBefore(LocalDate.now())) {
            throw new BadRequestAlertException("Appointment date must be today or later", "appointment",
                    "invalid_date");
        }

        // validate patient and doctor
        patientRepository.findById(request.getPatientId())
                .orElseThrow(() -> new BadRequestAlertException("Patient not found: " + request.getPatientId(),
                        "appointment", "patientnotfound"));
        if (!patientRepository.existsById(request.getPatientId())) {
            throw new BadRequestAlertException("Patient not found: " + request.getPatientId(), "appointment",
                    "patientnotfound");
        }
        if (!employeeRepository.existsById(request.getDoctorId())) {
            throw new BadRequestAlertException("Doctor not found: " + request.getDoctorId(), "appointment",
                    "doctornotfound");
        }

        employeeRepository.findById(request.getDoctorId())
                .orElseThrow(() -> new BadRequestAlertException("Doctor not found: " + request.getDoctorId(),
                        "appointment", "doctornotfound"));

        // check overlap
        List<Appointment> overlaps = repository.findOverlappingByDoctorAndDate(request.getDoctorId(),
                request.getAppointmentDate(), request.getStartTime(), request.getEndTime());
        if (!overlaps.isEmpty()) {
            throw new BadRequestAlertException("Doctor not available for the given time slot", "appointment",
                    "overlap");
        }

        // Check dentist work schedules for availability on that date
        java.util.List<DentistWorkSchedule> schedules = dentistWorkScheduleRepository
                .findByDentistIdAndWorkDate(request.getDoctorId(), request.getAppointmentDate());
        if (schedules == null || schedules.isEmpty()) {
            throw new BadRequestAlertException("Doctor is not available on this date", "appointment",
                    "doctor_not_available");
        }

        boolean withinWorkingHours = schedules.stream().anyMatch(s -> !s.getStartTime().isAfter(request.getStartTime())
                && !s.getEndTime().isBefore(request.getEndTime()));
        if (!withinWorkingHours) {
            throw new BadRequestAlertException("Appointment time is outside doctor's working hours", "appointment",
        ...
        long seq = repository.countByDoctorIdAndAppointmentDate(request.getDoctorId(), request.getAppointmentDate())
                + 1;
    ...
    public AppointmentResponse confirm(String appointmentId) {
        ...
        if (appt.getStatus() != AppointmentStatus.SCHEDULED) {
            throw new BadRequestAlertException("Only SCHEDULED appointments can be confirmed", "appointment",
                    "invalid_status");
        }
    ...
    public AppointmentResponse complete(String appointmentId) {
        ...
        if (appt.getStatus() != AppointmentStatus.CONFIRMED) {
            throw new BadRequestAlertException("Only CONFIRMED appointments can be completed", "appointment",
                    "invalid_status");
        }
    ...
    public AppointmentResponse noShow(String appointmentId) {
        Appointment appt = repository.findById(appointmentId)
                .orElseThrow(() -> new BadRequestAlertException("Appointment not found: " + appointmentId,
                        "appointment", "notfound"));
        if (appt.getStatus() != AppointmentStatus.CONFIRMED) {
            throw new BadRequestAlertException("Only CONFIRMED appointments can be marked as no-show", "appointment",
                    "invalid_status");
        }
    ...
    public AppointmentResponse cancel(String appointmentId, String cancellationReason) {
        ...
        if (appt.getStatus() == AppointmentStatus.COMPLETED) {
            throw new BadRequestAlertException("Cannot cancel a completed appointment", "appointment",
                    "invalid_status");
        }
    ...
    public AppointmentResponse reschedule(String appointmentId, RescheduleAppointmentRequest request) {
        ...
        if (!(appt.getStatus() == AppointmentStatus.SCHEDULED || appt.getStatus() == AppointmentStatus.CONFIRMED)) {
            throw new BadRequestAlertException("Only SCHEDULED or CONFIRMED appointments can be rescheduled",
                    "appointment", "invalid_status");
        }
        ...
        java.util.List<DentistWorkSchedule> schedules = dentistWorkScheduleRepository
                .findByDentistIdAndWorkDate(appt.getDoctorId(), request.getAppointmentDate());
        if (schedules == null || schedules.isEmpty()) {
            throw new BadRequestAlertException("Doctor is not available on this date", "appointment",
                    "doctor_not_available");
        }
        ...
        List<Appointment> overlaps = repository.findOverlappingByDoctorAndDateExcluding(appt.getDoctorId(),
                request.getAppointmentDate(), request.getStartTime(), request.getEndTime(), appointmentId);
        if (!overlaps.isEmpty()) {
        ...
        String prev = String.format("Rescheduled from %s %s-%s. ", appt.getAppointmentDate(), appt.getStartTime(),
                appt.getEndTime());
    ...
    public AppointmentResponse update(String appointmentId, UpdateAppointmentRequest request) {
        ...
        if (existingStatus == AppointmentStatus.COMPLETED || existingStatus == AppointmentStatus.CANCELLED
                || existingStatus == AppointmentStatus.NO_SHOW) {
            throw new BadRequestAlertException("Appointment is final and cannot be modified", "appointment",
                    "final_status");
        }
        ...
        if (request.getStatus() != null) {
            ...
            if (!isValidStatusTransition(currentStatus, nextStatus)) {
                throw new BadRequestAlertException(
                        "Invalid status transition from " + currentStatus + " to " + nextStatus, "appointment",
                        "invalid_status_flow");
            }
        }
        ...
        if (resultingStatus != null && resultingStatus != AppointmentStatus.SCHEDULED) {
            if (request.getPatientId() != null && !request.getPatientId().equals(appt.getPatientId())) {
                throw new BadRequestAlertException("Cannot change patient_id when appointment status is not SCHEDULED",
                        "appointment", "immutable");
            }
            if (request.getDoctorId() != null && !request.getDoctorId().equals(appt.getDoctorId())) {
                throw new BadRequestAlertException("Cannot change doctor_id when appointment status is not SCHEDULED",
                        "appointment", "immutable");
            }
        }

        // Determine effective date/time/doctor for validation (use existing values when
        // not provided in request)
        LocalDate date = request.getAppointmentDate() != null ? request.getAppointmentDate()
        ...
        if (date == null || start == null || end == null) {
            throw new BadRequestAlertException("Appointment date/start/end must be provided", "appointment",
                    "incomplete_datetime");
        }

        // Check overlaps excluding the appointment being updated
        List<Appointment> overlaps = repository.findOverlappingByDoctorAndDateExcluding(doctorId, date, start, end,
                appointmentId);
        if (!overlaps.isEmpty()) {
            throw new BadRequestAlertException("Doctor not available for the given time slot", "appointment",
                    "overlap");
    ...
    /**
     * Validate allowed status transitions. Returns true if transitioning from
     * current -> next is permitted.
     */
    ...
    private boolean isValidStatusTransition(AppointmentStatus current, AppointmentStatus next) {
        ...
        switch (current) {
            case SCHEDULED:
                ...
                return next == AppointmentStatus.SCHEDULED || next == AppointmentStatus.CONFIRMED
            ...
            case CONFIRMED:
                ...
                // - go to CANCELLED
                // - go to NO_SHOW
                return next == AppointmentStatus.CONFIRMED || next == AppointmentStatus.COMPLETED
                        || next == AppointmentStatus.CANCELLED || next == AppointmentStatus.NO_SHOW;
            default:
                return false;
        }
    }
