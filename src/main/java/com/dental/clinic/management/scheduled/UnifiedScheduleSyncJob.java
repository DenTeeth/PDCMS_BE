 ...
 * 
 * Purpose: Read schedules from BOTH sources (Fixed & Flex) and sync to employee_shifts.
 * 
 * Runs: Daily at 00:01 AM (self-healing - auto-corrects within 24 hours if admin changes schedules)
 * 
 * Business Logic:
 * 1. Define 14-day window: [Today] to [Today + 14 days]
 * 2. Clean old SCHEDULED shifts in this window (from BATCH_JOB & REGISTRATION_JOB sources)
 * 3. Loop 14 times (Day 0 to Day 13):
 *    - Query 1: Get Fixed schedules (fixed_shift_registrations + fixed_registration_days)
 *    - Query 2: Get Flex schedules (employee_shift_registrations + part_time_slots)
 *    - Merge results
...
public class UnifiedScheduleSyncJob {
     ...
     * Deletes shifts with status=SCHEDULED and source=BATCH_JOB or REGISTRATION_JOB.
        ...
        try {
            ...
            for (EmployeeShiftRegistration registration : allFlex) {
                ...
                com.dental.clinic.management.employee.domain.Employee employee = 
                    employeeRepo.findById(registration.getEmployeeId()).orElse(null);
                if (employee == null) {
