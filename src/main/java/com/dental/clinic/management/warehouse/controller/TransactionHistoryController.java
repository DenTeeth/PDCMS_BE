 ...
 *
 * Features:
 * - Comprehensive filtering (type, status, payment, date, supplier, appointment)
...
public class TransactionHistoryController {
    ...
    public ResponseEntity<TransactionHistoryResponse> getTransactionHistory(
            ...
            @Parameter(description = "Tìm kiếm theo mã phiếu (PN-xxx, PX-xxx) hoặc số hóa đơn")
            @RequestParam(required = false) String search,

            @Parameter(description = "Loại phiếu: IMPORT, EXPORT, ADJUSTMENT")
            @RequestParam(required = false) TransactionType type,

            @Parameter(description = "Trạng thái duyệt: DRAFT, PENDING_APPROVAL, APPROVED, REJECTED, CANCELLED")
            @RequestParam(required = false) TransactionStatus status,

            @Parameter(description = "Trạng thái thanh toán (chỉ IMPORT): UNPAID, PARTIAL, PAID")
            @RequestParam(required = false) PaymentStatus paymentStatus,

            @Parameter(description = "Lấy giao dịch từ ngày (YYYY-MM-DD)")
            @RequestParam(required = false) @DateTimeFormat(iso = DateTimeFormat.ISO.DATE) LocalDate fromDate,

            @Parameter(description = "Lấy giao dịch đến ngày (YYYY-MM-DD)")
            @RequestParam(required = false) @DateTimeFormat(iso = DateTimeFormat.ISO.DATE) LocalDate toDate,

            @Parameter(description = "Lọc theo nhà cung cấp (chỉ IMPORT)")
            @RequestParam(required = false) Long supplierId,

            @Parameter(description = "Lọc theo ca điều trị (chỉ EXPORT)")
            @RequestParam(required = false) Long appointmentId,

            @Parameter(description = "Lọc theo người tạo (employee_id)")
