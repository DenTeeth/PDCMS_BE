...
public class AdminLeaveBalanceController {
     ...
     *
     * Authorization: VIEW_LEAVE_BALANCE
     *
     * Query Params:
     * - cycle_year (integer, optional): Lọc theo năm. Mặc định là năm hiện tại
     * - time_off_type_id (string, optional): Lọc theo một loại phép cụ thể (ví dụ:
     * ANNUAL_LEAVE)
     *
     * Response:
    ...
    public ResponseEntity<AllEmployeesLeaveBalanceResponse> getAllEmployeesLeaveBalances(
        ...
        AllEmployeesLeaveBalanceResponse response = balanceService.getAllEmployeesLeaveBalances(cycleYear,
                timeOffTypeId);
     ...
     *
     * Query Params:
     * - cycle_year (integer, optional): Năm muốn xem. Nếu để trống, mặc định là năm
     * hiện tại
     *
     * Response:
     ...
     * - Kiểm tra xem đã có employee_leave_balances cho apply_to_type_id và
     * cycle_year chưa
     * - Nếu CHƯA có:
     ...
     */
    @Operation(summary = "Annual leave balance reset", description = "CRON job to automatically reset annual leave balances for all active employees at year start")
