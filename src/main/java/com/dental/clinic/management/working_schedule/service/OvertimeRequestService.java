...
public class OvertimeRequestService {
        ...
        public OvertimeRequestDetailResponse createOvertimeRequest(CreateOvertimeRequestDTO dto) {
                ...
                WorkShift workShift = workShiftRepository.findById(dto.getWorkShiftId())
                                .orElseThrow(() -> new RelatedResourceNotFoundException("Ca làm việc",
                                                dto.getWorkShiftId()));
                ...
                if (dto.getWorkDate().isBefore(today)) {
                        ...
                        throw new IllegalArgumentException(
                                        "Ngày làm việc không được là quá khứ.");
                }
                ...
                if (dto.getWorkDate().isEqual(today)) {
                        ...
                        if (workShift.getEndTime().isBefore(now) || workShift.getEndTime().equals(now)) {
                                ...
                                throw new IllegalArgumentException(
                                                String.format(
                                                                "Ca làm việc đã kết thúc (kết thúc lúc %s). Không thể tạo yêu cầu OT.",
                ...
                // Validation 5: Check for time-overlapping shifts on the same date (Lỗi
                // 3)
                ...
                for (OvertimeRequest existingRequest : existingRequests) {
                        ...
                        if (existingRequest.getStatus() == RequestStatus.REJECTED &&
                                ...
                                throw new IllegalArgumentException(
                                                String.format(
                                                                "Yêu cầu OT cho ca này đã bị từ chối (lý do: %s). Vui lòng liên hệ quản lý nếu cần thay đổi.",
                                                                existingRequest.getRejectedReason() != null
                                                                                ? existingRequest.getRejectedReason()
                                                                                : "Không rõ"));
                        }
                        ...
                        if (existingRequest.getStatus() == RequestStatus.CANCELLED &&
                                ...
                                throw new IllegalArgumentException(
                                                String.format(
                                                                "Yêu cầu OT cho ca này đã bị hủy trước đó (lý do: %s). Vui lòng liên hệ quản lý nếu cần tạo lại.",
                        ...
                        if (timeOverlap) {
                                log.warn(
                                                ...
                                                workShift.getWorkShiftId(), workShift.getStartTime(),
                                                workShift.getEndTime());
                                throw new IllegalArgumentException(
                                                String.format(
                                                                "Nhân viên đã có ca OT trùng giờ trong ngày %s (ca %s: %s-%s). Không thể tạo ca OT mới (ca %s: %s-%s).",
                                                                dto.getWorkDate(),
                                                                existingShift.getShiftName(),
                                                                existingShift.getStartTime(),
                                                                existingShift.getEndTime(),
                                                                workShift.getShiftName(), workShift.getStartTime(),
                                                                workShift.getEndTime()));
                        }
                }
                ...
                if (hasExactDuplicate) {
                        log.warn("Exact duplicate overtime request exists for employee {} on {} shift {}",
                                        targetEmployeeId, dto.getWorkDate(), dto.getWorkShiftId());
                        throw new DuplicateOvertimeRequestException(targetEmployeeId, dto.getWorkDate(),
                                        dto.getWorkShiftId());
                }
        ...
        public OvertimeRequestDetailResponse updateOvertimeStatus(String requestId, UpdateOvertimeStatusDTO dto) {
                ...
                switch (dto.getStatus()) {
                        ...
                        default ->
                                throw new IllegalArgumentException(
                                                "Trạng thái không hợp lệ: " + dto.getStatus());
                }
        ...
        private void handleApproval(OvertimeRequest request, Employee approvedBy) {
                ...
                if (!SecurityUtil.hasCurrentUserPermission("APPROVE_OT")) {
                        ...
                        throw new AccessDeniedException(
                                        "Bạn không có quyền duyệt yêu cầu OT.");
                }
        ...
        private void handleRejection(OvertimeRequest request, UpdateOvertimeStatusDTO dto, Employee rejectedBy) {
                ...
                if (!SecurityUtil.hasCurrentUserPermission("REJECT_OT")) {
                        ...
                        throw new AccessDeniedException(
                                        "Bạn không có quyền từ chối yêu cầu OT.");
                }
        ...
        private void handleCancellation(OvertimeRequest request, UpdateOvertimeStatusDTO dto, Employee cancelledBy) {
                ...
                if (!canCancelOwn && !canCancelAny) {
                        ...
                        throw new AccessDeniedException(
                                        "Bạn không có quyền hủy yêu cầu OT này.");
                }
        ...
        private void createEmployeeShiftFromOvertimeApproval(OvertimeRequest request) {
                try {
                        ...
                        employeeShift.setCreatedBy(request.getApprovedBy().getEmployeeId()); // Track who approved
                                                                                             // (created shift)
                        employeeShift.setSourceOtRequestId(request.getRequestId()); // Link to OT request
                        employeeShift.setNotes(String.format("Tạo tác vụ cho yêu cầu OT %s - %s",
        ...
        private void validateNoScheduleConflict(Integer employeeId, LocalDate workDate, String workShiftId) {
                ...
                if (hasFixedSchedule) {
                        ...
                        throw new SlotConflictException(
                                        "Nhân viên đã có lịch làm việc bình thường vào ca này.");
                }
