 ...
 * 
 * EXECUTION ORDER:
 * 1. spring.sql.init runs dental-clinic-seed-data.sql (creates ENUMs only, with continue-on-error=true)
 ...
 * No additional SQL files are created - we parse the same file and skip CREATE TYPE statements.
 */
@Slf4j
@Configuration
    ...
    public void initData() {
        try {
            ...
            if (roleCount != null && roleCount > 0) {
                log.info("Partial seed data detected (roles exist but items missing), will attempt to load missing data...");
            }
                ...
                for (String statement : statements) {
                    ...
                    if (upperStatement.startsWith("INSERT") ||
                        ...
                        try {
                            ...
                            // Log but continue (some statements might fail due to FK constraints - that's OK)
                            if (executedCount < 10) {
