...
public class TreatmentPlanController {
    ...
    private final com.dental.clinic.management.treatment_plans.service.CustomTreatmentPlanService customTreatmentPlanService;        /**
         ...
         * <p>
         * Key Features:
         * - Quantity Expansion: Setting quantity=5 creates 5 separate items with auto-incremented sequence
         * - Price Override: Allows custom pricing (must be within 50%-150% of service default)
         * - Approval Workflow: Created plans have approval_status=DRAFT (requires manager approval before activation)
         ...
         * 5. Expands items by quantity (e.g., quantity=3 → creates 3 items with sequence 1,2,3)
         ...
         *
         * @param patientCode Patient's business key (e.g., "BN-1001")
         * @param request     Request body with phases, items, quantities, and custom prices
         * @return Newly created custom treatment plan with complete details (201 CREATED)
         */
         ...
         * - Admin (VIEW_TREATMENT_PLAN_ALL): Can filter by doctorEmployeeCode/patientCode, sees ALL plans
         ...
         */
        @Operation(
                summary = "Get all treatment plans with advanced filtering (API 5.5)",
                description = """
        ...
        public ResponseEntity<Page<TreatmentPlanSummaryDTO>> getAllTreatmentPlans(
                @Parameter(description = "Filter by treatment plan status", example = "ACTIVE")
                @RequestParam(required = false) com.dental.clinic.management.treatment_plans.enums.TreatmentPlanStatus status,

                @Parameter(description = "Filter by approval status (V19)", example = "APPROVED")
                @RequestParam(required = false) com.dental.clinic.management.treatment_plans.domain.ApprovalStatus approvalStatus,

                @Parameter(description = "Filter by plan code (starts with)", example = "PLAN-20250112")
                @RequestParam(required = false) String planCode,

                @Parameter(description = "Filter by doctor employee code (Admin only)", example = "EMP001")
                @RequestParam(required = false) String doctorEmployeeCode,

                @Parameter(description = "Filter by patient code (Admin only)", example = "BN-1001")
                @RequestParam(required = false) String patientCode,

                @Parameter(description = "Filter start date FROM (yyyy-MM-dd)", example = "2025-01-01")
                ...
                @Parameter(description = "Filter start date TO (yyyy-MM-dd)", example = "2025-12-31")
                @RequestParam(required = false) @org.springframework.format.annotation.DateTimeFormat(iso = org.springframework.format.annotation.DateTimeFormat.ISO.DATE) java.time.LocalDate startDateTo,

                @Parameter(description = "Filter created date FROM (yyyy-MM-dd)", example = "2025-01-01")
                @RequestParam(required = false) @org.springframework.format.annotation.DateTimeFormat(iso = org.springframework.format.annotation.DateTimeFormat.ISO.DATE) java.time.LocalDate createdAtFrom,

                @Parameter(description = "Filter created date TO (yyyy-MM-dd)", example = "2025-12-31")
                @RequestParam(required = false) @org.springframework.format.annotation.DateTimeFormat(iso = org.springframework.format.annotation.DateTimeFormat.ISO.DATE) java.time.LocalDate createdAtTo,

                @Parameter(description = "Search term (plan name, patient name)", example = "orthodontics")
                @RequestParam(required = false) String searchTerm,

                @Parameter(description = "Pagination parameters (page=0, size=20, sort=createdAt,desc)")
                Pageable pageable
        ) {
                ...
                com.dental.clinic.management.treatment_plans.dto.request.GetAllTreatmentPlansRequest request =
                        com.dental.clinic.management.treatment_plans.dto.request.GetAllTreatmentPlansRequest.builder()
                                .status(status)
                ...
                log.info("Returning {} treatment plans (page {}/{}, total: {})",
                        plans.getNumberOfElements(), plans.getNumber() + 1, plans.getTotalPages(), plans.getTotalElements());
